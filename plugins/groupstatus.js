...
async function gcsstatusCommand(sock, chatId, senderId, message, args) {
    try {
            ...
            if (mediaKey) {
                ...
                if (messageType && content[messageType]) {
                    content[messageType].contextInfo = {
                        ...
                        statusAttributions: [{groupStatus: {authorJid: senderId}}],
                        forwardingScore: 999,
                        isForwarded: true,
                ...
                content = {
                    extendedTextMessage: {
                        text: statusText,
                        backgroundArgb: 4278241280, // Black color
                        font: 1,
        ...
        const startUI = `��� ������ — �����: ${targetGroupJids.length} ������`;
        ...
        for (let i = 0; i < targetGroupJids.length; i += batchSize) {
            ...
            if (progressMsg) {
                await sock.sendMessage(chatId, {
                    text: `��� ������ — ����������: ${i + 1}–${Math.min(i + batchSize, targetGroupJids.length)} 𝘰𝘧 ${targetGroupJids.length}`,
...
cmd({
    ...
    try {
        ...
        await gcsstatusCommand(conn, chatId, senderId, mek, args);
    } catch (err) {
        console.error('[GROUP-STATUS] Wrapper error:', err);
        await conn.sendMessage(mek.key.remoteJid, {
            text: `❌ 𝑮𝒓𝒐𝒖𝒑 𝑺𝒕𝒂𝒕𝒖𝒔 𝑬𝒓�𝒐𝒓\n���𝒆𝒕𝒉𝒊𝒏𝒈 𝒘𝒆𝒏𝒕 𝒘𝒓𝒐��. ��𝒆𝒂𝒔� 𝒕𝒓� 𝒂����.`,
            quoted: mek
        });
    }
});
